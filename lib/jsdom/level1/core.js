...
core.DOMException = function(code, message) {
  ...
  if(Error.captureStackTrace) {
    Error.captureStackTrace(this, core.DOMException);
  }
};
...
core.DOMImplementation = function DOMImplementation(/* Document */ document,
                                                     /* Object */ features)
{
...
core.Node.prototype = {
  ...
  get nodeName() {
    ...
    if (this.nodeType === ELEMENT_NODE &&
        ...
        this._ownerDocument._doctype.name.toLowerCase() === "html")
    {
  ...
  _indexOf: function(/*Node*/ child) {
    ...
    var childNodes = this._childNodes, len = childNodes.length;

    while (len--) {
      if (childNodes[len] == child) {
        return len;
      }
    }
  ...
  replaceChild : function(/* Node */ newChild, /* Node */ oldChild){
    // document.doctype isn't a child, so special treatment is required
    if (this.nodeType === this.DOCUMENT_NODE &&
        oldChild.nodeType === this.DOCUMENT_TYPE_NODE &&
        newChild.nodeType === this.DOCUMENT_TYPE_NODE)
    {
      this._doctype = newChild;
      return oldChild;
    } else {
  ...
  cloneNode : function(/* bool */ deep, fn) {
    ...
    switch (this.nodeType) {
      ...
      case this.ELEMENT_NODE:
        object = attrCopy(this,
                          this._ownerDocument.createElement(this.tagName), fn);
      break;

      case this.TEXT_NODE:
        object = attrCopy(this,
                          this._ownerDocument.createTextNode(this.tagName));
        object.nodeValue = this.nodeValue;
      break;util.mapper(this, filterByTagName, true)
      ...
      case this.DOCUMENT_NODE:
        ...
        object._doctype = this.doctype.cloneNode(true);
      break;
    ...
    object._ownerDocument = this._ownerDocument;
  ...
  normalize: function() {
    ...
    for (var j=0, k=this._childNodes.length; j<k; j++) {
      child = this._childNodes[j];
      ...
      if (j>0) {
        prevChild = this._childNodes[j-1];
...
core.Element.prototype = {
  ...
  getAttributeNode: function(/* string */ name) {
    ...
    // attempt to look it up a default attribute
    if (!attr && this._ownerDocument) {
      var doc = this._ownerDocument;
      if (doc._doctype && doc._doctype._attributes) {
        var el = doc._doctype._attributes.getNamedItem(this._nodeName);
        if (el) {
          attr = el._attributes.getNamedItem(name);
        }
      }
    }
  ...
  getElementsByTagName: function(/* string */ name) {
    ...
    var el = this._ownerDocument || this;

    return new core.NodeList(el, util.mapper(this, filterByTagName, true));
  },
};
...
core.ProcessingInstruction = function ProcessingInstruction(document,
                                                             target,
                                                             data)
{
  this._ownerDocument = document;
...
core.Document.prototype = {
  ...
  createElement: function(/* string */ tagName) {
    ...
    if (!tagName || !tagName.match || !util.validate(this, 'tag', tagName)) {
      throw new core.DOMException(INVALID_CHARACTER_ERR,
                                   'Invalid character in tag name: ' + tagName);
    }

    var builder = (this._elementBuilders[tagName.toLowerCase()] ||
                    this._defaultElementBuilder);

    element = builder(this, tagName);

    // Check for and introduce default elements
    if (this._doctype             &&
        this._doctype._attributes &&
        this._doctype.name.toLowerCase() !== "html")
    {
  ...
  createProcessingInstruction: function(/* string */ target,
                                         /* string */ data)
  {
    ...
    if (!target || !target.length || !util.validate(this, 'tag', target)) {
      throw new core.DOMException(INVALID_CHARACTER_ERR, "tag name:", target);
    }
  ...
  createAttribute: function(/* string */ name) {
    if (!name || !name.length || !util.validate(this, 'attribute', name)) {
      throw new core.DOMException(INVALID_CHARACTER_ERR,
                                   "attribute name:", name);
    }
  ...
  createEntityReference: function(/* string */ name) {
    ...
    if (!name || !name.length) {
      throw new core.DOMException(INVALID_CHARACTER_ERR,
                                   'entity reference name:', name);
    }

    if (!util.validate(this, 'tag', name)) {
      throw new core.DOMException(INVALID_CHARACTER_ERR, 'tag name:', name);
    }

    var entity;
    // must use the getter here.
    if (this.doctype && this.doctype.entities) {
  ...
  createEntityNode : function(/* string */ name)
  {
    ...
    if (!name || !name.length || !util.validate(this, 'entity', name)) {
      throw new core.DOMException(INVALID_CHARACTER_ERR, 'entity:', name);
    }

    // entity nodes are marked readonly by default
    ...
    util.markTreeReadonly(ret);

    return ret;
  },

  /* returns Notation */
  createNotationNode : function(/* string */ name,
                                 /* string */ publicId,
                                 /* string */ systemId)
  {

    if (!name || !name.length || !util.validate(this, 'entity', name)) {
      throw new core.DOMException(INVALID_CHARACTER_ERR, 'notation:', name);
    }

    var ret = new core.Notation(this, name, publicId, systemId);

    // Notation nodes are marked readonly by default
    ret._readonly = false;
  ...
  getElementsByTagName: function(/* string */ name) {
    function filterByTagName(child) {
      ...
      if (child && child.nodeType && child.nodeType === ENTITY_REFERENCE_NODE)
      {
        child = child._entity;
      }


      if (child && child.nodeName && child.nodeType === ELEMENT_NODE)
      {
        if (name === "*") {
          return true;
    ...
    var el = this.documentElement || this;
...
core.DocumentType = function DocumentType(document,
                                           name,
                                           entities,
                                           notations,
                                           attributes)
{
  ...
  util.markTreeReadonly(this._notations);
...
core.EntityReference.prototype = {
  ...
  get childNodes() { return this._entity ? this._entity.childNodes : null },
