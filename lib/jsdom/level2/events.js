...
util.intercept(core.Node, 'insertBefore', function(_super, args, newChild, refChild) {
  ...
  if (mutationEventsEnabled(this)) {
    ...
    if (this.nodeType == core.Node.DOCUMENT_NODE || this._attachedToDocument) {
      ...
      util.visitTree(newChild, function(el) {
...
util.intercept(core.Node, 'removeChild', function (_super, args, oldChild) {
  if (mutationEventsEnabled(this)) {
    var doc = getDocument(this),
        ev = doc.createEvent("MutationEvents");
    ...
    util.visitTree(oldChild, function(el) {
      if (el.nodeType == core.Node.ELEMENT_NODE) {
        el.dispatchEvent(ev);
...
util.intercept(core.AttrNodeMap, 'removeNamedItem', dispatchAttrEvent('REMOVAL'));
util.intercept(core.AttrNodeMap, 'setNamedItem', dispatchAttrEvent('ADDITION'));
