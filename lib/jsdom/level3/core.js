...
core.Node.prototype.lookupNamespaceURI = function (prefix) {
  switch (this.nodeType) {
    case this.ELEMENT_NODE:
      ...
      if (attributes) {
        var attrIndex = attributes.length;
        var attr;
        while (attrIndex--) {
          attr = attributes[attrIndex];
          // non default namespace
          if (attr.prefix === "xmlns" && attr.localName == prefix ) {
            return attr.nodeValue || null;
          }
          // default namespace
          else if (attr.localName == "xmlns" && attr.prefix === null) {
            return attr.nodeValue || null;
          }
        }
...
core.Node.prototype._triggerUserDataHandler = function(operationType, dest) {
  var that = this;
  if (this._userDataHandlers && this._userData && !this._pauseUserDataHandler) {
    Object.keys(this._userDataHandlers).forEach(function(handler) {
      that._userDataHandlers[handler](
        operationType,
        handler,
        that._userData[handler],
        that,
        dest || null
      );
    });
...
core.Node.prototype._userDataHandlers = null;
core.Node.prototype._userData = null;
core.Node.prototype._pauseUserDataHandler = false;
core.Node.prototype.setUserData = function (key, data, handler) {
  if (!this._userDataHandlers) {
    this._userDataHandlers = {};
  }
  ...
  var r = this._userData[key] || null;
  this._userData[key] = data;

  if (typeof handler === 'function') {
    this._userDataHandlers[key] = handler;
  } else if (this._userDataHandlers[key]) {
    delete this._userDataHandlers[key];
  }

  var keys = Object.keys(this._userDataHandlers);
  return r;
};

core.Node.prototype.getUserData = function (key) {
  return (this._userData) ? this._userData[key] || null : null;
};

(function(_cloneNode) {
  core.Node.prototype.cloneNode = function(deep) {
    var cloned =  _cloneNode.call(this, deep);
    this._triggerUserDataHandler(core.UserDataHandler.NODE_CLONED, cloned);
    return cloned;
  };
})(core.Node.prototype.cloneNode);


(function(_importNode) {
  core.Document.prototype.importNode = function(importedNode, deep) {
    // Since the original importNode uses cloneNode on the src node
    // silence the user data handler during the clone
    importedNode._pauseUserDataHandler = true;
    var imported =  _importNode.call(this, importedNode, deep);
    importedNode._pauseUserDataHandler = false;

    // trigger node imported message
    importedNode._triggerUserDataHandler(core.UserDataHandler.NODE_IMPORTED, imported);
    return imported;
  };
})(core.Document.prototype.importNode);
...
core.UserDataHandler.NODE_CLONED = 1;
core.UserDataHandler.NODE_IMPORTED = 2;
core.UserDataHandler.NODE_DELETED  = 3;
core.UserDataHandler.NODE_RENAMED  = 4;
core.UserDataHandler.NODE_ADOPTED  = 5;
core.UserDataHandler.prototype = {
  NODE_CLONED   : 1,
  NODE_IMPORTED : 2,
  NODE_DELETED  : 3,
  NODE_RENAMED  : 4,
  NODE_ADOPTED  : 5,
  handle : function (operation, key, data, src, dst) {}
};
...
(function(_removeChild) {
  core.Document.prototype.removeChild = function(child) {
    if (child.nodeType === this.DOCUMENT_TYPE_NODE) {
      this._doctype = null;
    }
    return _removeChild.call(this, child);
  };
})(core.Document.prototype.removeChild);
...
core.Document.prototype.adoptNode = function (node) {
  ...
  node._triggerUserDataHandler(core.UserDataHandler.NODE_ADOPTED);
  return node;
};
...
core.Document.prototype.renameNode = function (node, namespaceURI, qualifiedName) {
  ...
  node._triggerUserDataHandler(core.UserDataHandler.NODE_RENAMED);
