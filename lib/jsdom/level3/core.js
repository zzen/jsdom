  ...
  'cdata-sections' : function(doc, els, val, error) {
    ...
    while (index--) {
      ...
      if (element.nodeType == CDATA) {
        var textNode = doc.createTextNode('CDATA');
...
console.log(element.namespaceURI, element.tagName);
      ...
      if (element.namespaceURI) {
        ...
        if (!element.localName) {
          ...
          if (doc.domConfig.getParameter('validate')) {
            error(new core.DOMError(core.DOMError.SEVERITY_FATAL_ERROR));
          } else {
            error(new core.DOMError(core.DOMError.SEVERITY_ERROR));
          }
        }
      }
    }
...
core.Document.prototype.documentURI = null;
